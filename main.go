...
import (
	...
	"github.com/osulumilous/go-sukori/modules/btcaddress"
	"github.com/osulumilous/go-sukori/modules/btcconversions"
	"github.com/osulumilous/go-sukori/routers/oauth"
	"github.com/osulumilous/go-sukori/routers/pagemappings"
	"github.com/osulumilous/go-sukori/services"
	"github.com/osulumilous/go-sukori/services/cieca"
...
func main() {
	err := agplwarning.Warn("ripple", "sukori")
	if err != nil {
		fmt.Println(err)
	}

	fmt.Println("sukori " + version)
	...
	case nil:
		// carry on
	case conf.ErrNoFile:
		conf.Export(config, "config.conf")
	...
	var configDefaults = map[*string]string{
		...
		&config.AvatarURL:        "https://a.lumilous.pw",
		&config.BaseURL:          "https://osu.lumilous.pw",
		&config.BanchoAPI:        "https://c.lumilous.pw",
		...
		&config.MailgunFrom:      `"o!lumilous" <noreply@lumilous.pw>`,
	}
...
func generateEngine() *gin.Engine {
	...
	r.GET("/help", func(c *gin.Context) {
		c.Redirect(301, "https://discord.lumilous.pw")
	})
...
Perhaps report this to a Lumilous developer?
